...
def run_real_time_match(agent_1: UserInputAgent, agent_2: Agent, max_timesteps=30*90, resolution=CameraResolution.LOW,reward_manager=None):
    pygame.init()
    ...
    env = WarehouseBrawl(resolution=resolution, train_mode=False,reward_manager=reward_manager)
    reward_manager.subscribe_signals(env)
    observations, _ = env.reset()
    obs_1 = observations[1]
    obs_2 = observations[0]
   ...
   # platform1 = env.objects["platform1"] #mohamed
    ...
    while running and timestep < max_timesteps:
        ...
        # Sample action space
        full_action = {0: action_2, 1: action_1}
        observations, rewards, terminated, truncated, info = env.step(full_action)
        obs_1 = observations[0]
        obs_2 = observations[1]
        cum_r0+=rewards
        if timestep % 30 == 0:
            print(f"rewards: r0: {cum_r0}")
            cum_r0=0
