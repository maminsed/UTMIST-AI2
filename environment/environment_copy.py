...
class WarehouseBrawl(MalachiteEnv[np.ndarray, np.ndarray, int]):
    ...
    def __init__(self, mode: RenderMode=RenderMode.RGB_ARRAY, resolution: CameraResolution=CameraResolution.LOW, train_mode: bool = False,spawners=True,reward_manager=None):
    ...
    def step(self, action: dict[int, np.ndarray]):
        ...
        if self.reward_manager is not None:
            self.rewards = self.reward_manager.process(self, 1 / 30.0)
        else:
    ...
    def _setup(self,spawners=True):
      ...
      #  spear_img = pygame.image.load("/content/Weapon Pool/spear.png")
        ...
        print(f"debug: 1341: spawners: {spawners}")
        if spawners:
